...
module.exports = function(server) {
  ...
  if(!server) {
    server = http.createServer(ecstatic(path.join(__dirname, 'www')));
  }
  ...
  function sendUpdate(options) {
    ...
    var update = {triggerPlayer: options.triggerPlayer, positions:{}, date: +new Date()}
  ...
  wss.on('connection', function(ws) {
    ...
    emitter.on('created', function() {
      ...
      emitter.on('state', function(state) {
        ...
        sendUpdate({triggerPlayer: emitter.id});
      })
    })
